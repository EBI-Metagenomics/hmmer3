                            ...
                            unsigned char const *end)
{
  ...
  if (end < *ptr + 14 * sizeof(uint64_t) + 2) defer_return(H3C_EPARSE);
  ...
  if ((rc = h3client_hmmd_zsetby_parse(&stats->Z_setby, ptr)))    defer_return(rc);
  if ((rc = h3client_hmmd_zsetby_parse(&stats->domZ_setby, ptr))) defer_return(rc);
                              ...
                              unsigned char const *end)
{
  ...
  if ((rc = parse_first_part(stats, ptr, end))) defer_return(rc);

  if (end < *ptr + sizeof(uint64_t)) defer_return(H3C_EPARSE);
  uint64_t hit_offset = h3client_eatu64(ptr);

  if (hit_offset == UINT64_MAX && stats->nhits > 0) defer_return(H3C_EPARSE);
  ...
  if (!stats->hit_offsets) defer_return(H3C_ENOMEM);

  if (end < *ptr + stats->nhits * sizeof(uint64_t)) defer_return(H3C_EPARSE);
