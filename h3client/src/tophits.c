...
void h3client_tophits_init(struct tophits *th) { memset(th, 0, sizeof(*th)); }
...
static int grow(struct tophits *th, unsigned nhits)
{
  ...
  {
    if ((rc = h3client_hit_init(th->hits + i))) goto cleanup;
    ++th->nhits;
  }
...
static void shrink(struct tophits *th, unsigned nhits)
{
  for (unsigned i = nhits; i < th->nhits; ++i)
    h3client_hit_cleanup(th->hits + i);

  th->nhits = nhits;
}

int h3client_tophits_setup(struct tophits *th, unsigned nhits)
{
...
void h3client_tophits_cleanup(struct tophits *th)
{
...
int h3client_tophits_pack(struct tophits const *th, struct lip_file *f)
{
  lip_write_array_size(f, 5);

  lip_write_map_size(f, 1);
  lip_write_cstr(f, "hits");
  if (!lip_write_array_size(f, th->nhits)) return H3CLIENT_EPACK;

  for (unsigned i = 0; i < th->nhits; ++i)
  {
    int rc = h3client_hit_pack(th->hits + i, f);
    if (rc) return rc;
  }
...
int h3client_tophits_unpack(struct tophits *th, struct lip_file *f)
{
  int rc = H3CLIENT_EUNPACK;

  if (!h3client_expect_array_size(f, 5)) goto cleanup;

  if (!h3client_expect_map_size(f, 1)) goto cleanup;
  if (!h3client_expect_key(f, "hits")) goto cleanup;

  unsigned size = 0;
  if (!lip_read_array_size(f, &size)) goto cleanup;

  if ((rc = h3client_tophits_setup(th, size))) goto cleanup;

  for (unsigned i = 0; i < th->nhits; ++i)
  {
    if ((rc = h3client_hit_unpack(th->hits + i, f))) goto cleanup;
  }
...
void h3client_tophits_print_targets(struct tophits const *th, FILE *f, double Z)
{
  ...
  h3client_echo(f, "  %22s  %22s  %8s", " --- full sequence ---",
                " --- best 1 domain ---", "-#dom-");

  h3client_echo(f, "  %9s %6s %5s  %9s %6s %5s  %5s %2s  %-*s %s", "E-value",
                " score", " bias", "E-value", " score", " bias", "  exp", "N",
                namew, "Model", "Description");

  h3client_echo(f, "  %9s %6s %5s  %9s %6s %5s  %5s %2s  %-*s %s", "-------",
                "------", "-----", "-------", "------", "-----", " ----", "--",
                namew, "--------", "-----------");
  ...
  for (unsigned i = 0; i < th->nhits; i++)
  {
    ...
    {
      h3client_echo(f, "  ------ inclusion threshold ------");
...
void h3client_tophits_print_domains(struct tophits const *th, FILE *f, double Z,
                                    double domZ)
{
  h3client_echo(f, "Domain annotation for each model (and alignments):");
  ...
  for (unsigned i = 0; i < th->nhits; i++)
  {
    ...
    h3client_echo(f, ">> %s  %-.*s", name, descw, hit->desc);

    if (hit->nreported == 0)
    {
      h3client_echo(f, "   [No individual domains that satisfy reporting "
    ...
    for (unsigned j = 0; j < hit->ndomains; j++)
    {
      ...
      h3client_echo(f, " %4.2f", prob_ali_res(dom));
    }
    ...
    for (unsigned j = 0; j < hit->ndomains; j++)
    {
      ...
      h3client_echo(f, ";  conditional E-value: %.2g", evalue(dom->lnP, domZ));

      h3client_alidisplay_print(&dom->ad, f);
      h3client_echo(f, "");
    }
  }
...
void h3client_tophits_print_targets_table(char const *qacc,
...
{
  h3client_echo(f, "#%*s %22s %40s %11s %11s %11s",
...
void h3client_tophits_print_domains_table(char const *qacc,
                                          struct tophits const *th, FILE *f,
                                          bool show_header, double Z,
                                          double domZ)
{
...
char const *h3client_tophits_hit_name(struct tophits const *th, unsigned idx)
{
  return th->hits[idx].name;
}

char const *h3client_tophits_hit_acc(struct tophits const *th, unsigned idx)
{
  return th->hits[idx].acc;
}

double h3client_tophits_hit_evalue_ln(struct tophits const *th, unsigned idx,
                                      double Z)
{
