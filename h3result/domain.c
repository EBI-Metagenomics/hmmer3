...
int h3r_domain_pack(struct domain const *x, struct lio_writer *f)
{
  ...
  if (write_int(f, x->ienv)) return H3RESULT_EPACK;
  if (write_int(f, x->jenv)) return H3RESULT_EPACK;

  if (write_int(f, x->iali)) return H3RESULT_EPACK;
  if (write_int(f, x->jali)) return H3RESULT_EPACK;

  if (write_float(f, x->envsc))         return H3RESULT_EPACK;
  if (write_float(f, x->domcorrection)) return H3RESULT_EPACK;
  if (write_float(f, x->dombias))       return H3RESULT_EPACK;
  if (write_float(f, x->oasc))          return H3RESULT_EPACK;
  if (write_float(f, x->bitscore))      return H3RESULT_EPACK;
  if (write_float(f, x->lnP))           return H3RESULT_EPACK;

  if (write_bool(f, x->is_reported)) return H3RESULT_EPACK;
  if (write_bool(f, x->is_included)) return H3RESULT_EPACK;

  if (write_array(f, x->pos_score_size)) return H3RESULT_EPACK;
  for (unsigned long i = 0; i < x->pos_score_size; i++)
  {
    if (write_float(f, x->pos_score[i])) return H3RESULT_EPACK;
  }
...
int h3r_domain_unpack(struct domain *x, struct lio_reader *f)
{
  ...
  if ((rc = h3r_domain_setup(x, size))) return H3RESULT_EUNPACK;
