...
static void *memccpy_musl(void *restrict dest, const void *restrict src, int c,
                          size_t n)
{
...
static int check_header(struct hmr_profile *prof)
{
    ...
    if (!(tok = posix_strtok_r(tmp, " ", &ptr))) return HMR_EPARSE;

    if (strcmp(tok, "HMMER3/f")) return HMR_EPARSE;

    if (!(tok = posix_strtok_r(NULL, " ", &ptr))) return HMR_EPARSE;
    ...
    if (posix_strtok_r(NULL, " ", &ptr)) return HMR_EPARSE;
